####*****************************************************************
#### This is the code for running epidemics through the networks
####*****************************************************************

### Get the Random network
...
#on seeding all 20 nodes repeating the process 200 times(We choose 200 based on the preliminary runs)
####*******************************************************************************************************
...
curateMyNetwork <- function(examplenet) {
  ...
  translationTable <<- data.frame(city=unique(examplenetLine), code=unique(examplenetLineNum))### This table will be used to march numbers in the random network to city names # Created this is a global variable. Not widely recommended
...
runSimulation <- function(examplenet, k = 0.3, p = 2) {
  ...
  dflist_stats <- list()
  epioutList <- list()
  N <-20 # Added by PB represents 20 initial seedings - each seeding in turn is tested
  for(ii in 1:N){
    ...
    epiout 	<- SEIR.simulator2(examplenet, N = N, 
                               ...
                               init = ii)# init is a modification in the function to specify the number of nodes on which disease will be seeded, this allows us to seed disease on each node rather than random
...
runSimulationsForPs <- function(examplenet, k, ps = seq(0.1, 2, by = 0.1)) {
  return(
    bind_rows(
      lapply(
        ps,
...
runSimulationsForKs <- function(examplenet, ks = seq(0.01, 0.31, by = 0.02), ps = seq(0.1, 2, by = 0.1)) {
  return(
    bind_rows(
      lapply(
        ks,
...
finalTable_empirical   <- get.edgelist(gve) %>% curateMyNetwork %>% runSimulationsForKs
finalTable_empirical$file   <- "Empirical network"
Sys.time() - t0

finalTable_gravity     <- get.edgelist(gv2) %>% curateMyNetwork %>% runSimulationsForKs
finalTable_gravity$file     <- "Gravity network"

finalTable_molecular   <- get.edgelist(g2 ) %>% curateMyNetwork %>% runSimulationsForKs
finalTable_molecular$file   <- "Molecular network"
...
finalTable <- bind_rows(finalTable_empirical, finalTable_gravity, finalTable_molecular, finalTable_random_emp, finalTable_random_grav, finalTable_random_mol)

finalTable_sq <- bind_rows(finalTable_empirical, finalTable_gravity_sq, finalTable_molecular, finalTable_random_emp, finalTable_random_grav_sq, finalTable_random_mol)
